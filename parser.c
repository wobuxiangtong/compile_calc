...
static double parse_term(){
    ...
    for (;;) {
        my_get_token(&token);
        if (token.kind != MUL_OPERATION_TOKEN && token.kind != DIV_OPERATION_TOKEN) {
            unget_token(&token);
            break;
        }
...
double parse_expression(){
    ...
    for(;;){
        my_get_token(&token);
        if (token.kind != ADD_OPERATION_TOKEN && token.kind != SUB_OPERATION_TOKEN) {
            unget_token(&token);
            break;
        }
        v2 = parse_term();
